...
for "_x" from 0 to (_count-1) do {
	...
	if (isClass _weap) then {
		if ((getnumber (_weap >> "scope") == 2) && (gettext (_weap >> "dlc") == "SPE")) then {
			...
			if (!isClass (_weap >> "LinkedItems")) then {
				if ((count(getarray (_weap >> "magazines")) !=0) && (gettext (_weap >> "dlc") == "SPE") ) then {
			...
			if ( isClass(_weap >> "LinkedItems" >> "LinkedItemsUnder") && !isClass(_weap >> "LinkedItems" >> "LinkedItemsAcc") && !isClass(_weap >> "LinkedItems" >> "LinkedItemsMuzzle") && !isClass(_weap >> "LinkedItems" >> "LinkedItemsOptic") && (gettext (_weap >> "dlc") == "SPE")) then {
...
for "_x" from 0 to (_count-1) do {
    ...
    if (isClass _item) then {
        if ((getnumber (_item >> "scope") == 2) && (gettext (_item >> "dlc") == "SPE")) then {
...
for "_x" from 0 to (_count-1) do {
	...
	if (isClass _item) then {
		if((getNumber (_item >> "value") == 5) && (gettext (_item >> "dlc") == "SPE")) then {
...
for "_x" from 0 to (_count-1) do {
  _item=((configFile >> "CfgMagazines") select _x);
	if (isClass _item) then {
		if (gettext (_item >> "type") == _chargeType && ["remote", configName _item] call BIS_fnc_inString && (gettext (_item >> "dlc") == "SPE")) then {
...
for "_x" from 0 to (_count-1) do {
    _item=((configFile >> "CfgMagazines") select _x);
	if (isClass _item) then {
		if((getNumber (_item >> "type") == 16 || getNumber (_item >> "type") == 256) && (gettext (_item >> "dlc") == "SPE")) then {
